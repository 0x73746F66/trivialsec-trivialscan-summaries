...
output: env init
	@echo -e $(bold)$(primary)report_graphs_arn$(clear) = $(shell terraform -chdir=plans output report_graphs_arn)
	@echo -e $(bold)$(primary)report_graphs_role$(clear) = $(shell terraform -chdir=plans output report_graphs_role)
	@echo -e $(bold)$(primary)report_graphs_role_arn$(clear) = $(shell terraform -chdir=plans output report_graphs_role_arn)
	@echo -e $(bold)$(primary)report_graphs_policy_arn$(clear) = $(shell terraform -chdir=plans output report_graphs_policy_arn)
...
tail-aws-logs: ## Install using pipx install awscliv2
	awsv2 logs tail "/aws/lambda/$(shell sed -e 's/\(.*\)/\L\1/' <<< "${APP_ENV}")-trivialscan-report-graphs" --follow
